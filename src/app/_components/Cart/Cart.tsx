...
export default function Cart() {
    ...
    if (isLoading) return <EmptyCart />;
    if (error) return <EmptyCart />;
    const cartMenus = data?.data?.cart?.menus || [];
                            ...
                            <div className="relative">
                                <div className="p-4 bg-gray-200 flex justify-between gap-3 rounded-xl shadow-2xl h-full">
                                    ...
                                    <div className="md:w-[35%]">
                                        ...
                                        <p className="text-sm font-semibold text-gray-600 pb-10">
                                            {cart.menuId.categoryId.name}
                                        </p>
                                    </div>
                                    <div className="w-[25%] md:w-[15%]">
                                        <div className="flex flex-col justify-between items-center h-full">
                                            ...
                                            <h4
                                                className="md:font-bold cursor-pointer text-sm md:text-xl text-red-600"
                                                onClick={() => {
                                                    if (window.confirm("هل انت متأكد من الحذف؟")) {
                                                        deleteOneItemFromCart({ menuId: cart.menuId._id, token });
                                                    }
                                                }}
                                            >
                                                حذف
                                        ...
                                        <div className="flex flex-col justify-between items-center h-full">
                                            <div>
                                                <button
                                                    className="cart-btn"
                                                    onClick={() =>
                                                        updateCart({ menuId: cart.menuId._id, action: "increase", token })
                                                    }
                                                >
                                                    +
                                                </button>
                                            ...
                                            <div>
                                                <button
                                                    className="cart-btn"
                                                    onClick={() =>
                                                        updateCart({ menuId: cart.menuId._id, action: "decrease", token })
                                                    }
                                                >
                                                    -
                                                </button>
                    ...
                    <div className="flex items-center justify-between sm:mt-4 mt-10">
                        ...
                        <h4 className="font-bold text-sm px-1 md:text-xl text-center">
                            ...
                            <span className="text-red-500">
                                {data.data.cart.menus.reduce(
                                    (sum: number, menu: CartItem) =>
                                        sum + menu.menuId.finalPrice * menu.quntity,
                                    0
                                ).toFixed(2)}{" "}
                                جنيه
                            </span>
